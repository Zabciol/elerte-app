...
const countOfEmployeesOnDepartment = async (departmentID, dateArray) => {
  try {
    // Generowanie serii dat
    const datesForSQL = dateArray
      .map((date) => `SELECT '${date}-01' AS StartOfMonth`)
    ...
    const query = `
      ...
      UniqueEmployeeCounts AS (
        ...
        JOIN 
          ECP ON ECP.Pracownik_ID = Pracownicy.ID 
        JOIN 
          Stanowisko ON Pracownicy.Stanowisko_ID = Stanowisko.ID
        JOIN
          Dzialy ON Stanowisko.Dzial_ID = Dzialy.ID
        WHERE 
          Dzialy.ID = ?
        GROUP BY 
          YEAR(ECP.Data), MONTH(ECP.Data), Pracownicy.ID
    ...
    return await queryDatabasePromise(query, departmentID);
  } catch (error) {
    console.log(error);
    throw new Error(error);
  }
